...
public final class ParallelDownloader {
    ...
    public void download(final String urlString,
        ...
        if (fileSize <= 0) {
            throw new IOException("Invalid file size returned from remote " +
                                  "host:  " + fileSize);
        }

        final List<Callable<byte[]>> partitions =
        ...
        for (int i = 0; i < numChunks; i++) {
            final int finalI = i;
            final int start =
                (int)(Math.floor((double)fileSize / numChunks)) * finalI;

            //if this is last chunk set end equal to file size,
            // else just calculate end
            final int end = (i == (numChunks - 1))
                          ? fileSize - 1
                          : (int)((Math.floor((double)fileSize / numChunks)) *
                                              (finalI + 1) - 1);
            ...
            partitions.add(new Callable<byte[]>() {
                public byte[] call() {
                    byte[] bytes = null;
                    try {
                        bytes = Downloader.downloadChunk(start,
                                                         end,
                                                         urlString,
                                                         finalI);
        ...
        final ExecutorService executorPool =
            Executors.newFixedThreadPool(numChunks);
        final List<Future<byte[]>> results =
        ...
        for (final Future<byte[]> result : results) {
            ...
            System.arraycopy(result.get(),
                             0,
                             newByteArray,
                             0,
                             result.get().length);
            final byte[] tempByteArray =
                new byte[finalByteArray.length];

            System.arraycopy(finalByteArray,
                             0,
                             tempByteArray,
                             0,
            ...
            finalByteArray =
                new byte[newByteArray.length + tempByteArray.length];

            System.arraycopy(tempByteArray,
                             0,
                             finalByteArray,
                             0,
                             tempByteArray.length);

            System.arraycopy(newByteArray,
                             0,
                             finalByteArray,
                             tempByteArray.length,
                             newByteArray.length);
